...
export class DevspaceMessageComponent {
  ...
  // }
  onInputChange(value: string) {
    const lastAtIndex = value.lastIndexOf('@');  // Sucht nach dem letzten @-Zeichen
    const lastHashIndex = value.lastIndexOf('#');  // Sucht nach dem letzten #-Zeichen

    // Überprüft, ob hinter dem @ noch kein Name steht
    if (lastAtIndex !== -1 && value.slice(lastAtIndex + 1).trim() === '') {
      this.openContactBarSearch = true;  // Öffne die Kontakt-Suchleiste
    } else {
      this.openContactBarSearch = false;  // Schließe die Kontakt-Suchleiste
    }

    // Überprüft, ob hinter dem # noch kein Channel steht
    if (lastHashIndex !== -1 && value.slice(lastHashIndex + 1).trim() === '') {
      this.openChannelBarSearch = true;  // Öffne die Channel-Suchleiste
    } else {
      this.openChannelBarSearch = false;  // Schließe die Channel-Suchleiste
    }
  }
